...
(function () {
  ...
  style.textContent = \`
    ...
    #__tb_panel h3 { margin:0 0 8px; color:#e94560; font-size:15px; }
  ...
  document.addEventListener('mouseover', function (e) {
    ...
    e.target.classList.add('__tb_hover');
  }, true);
...
async function main() {
  ...
  await page.exposeFunction('__tb_start', () => {
    ...
    console.log(`[${ts()}] Polling started for "${markedText}" every ${POLL_MS} ms`);
    startPolling();
  });
  ...
  async function injectUI() {
    try { await page.evaluate(PICKER_SCRIPT); } catch (_) { /* navigating */ }
  }

  // Re-inject after every navigation (SPA route changes or reloads)
  page.on('load',  () => injectUI());
  page.on('crash', async () => { isChecking = false; await loadPage(); });

  // ── Start ─────────────────────────────────────────────────────────────────
  console.log(`[${ts()}] Launching browser…`);
  console.log('  Step 1 — Hover over the button you want to watch (red outline).');
